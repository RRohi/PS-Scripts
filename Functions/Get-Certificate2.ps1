...
.NOTES
The initial base for this function came from the Slogmeister Extraordinaire's answer in Stack Overflow:
...
Begin {
    ...
    ## Create a delimiter variable. This is necessary for external tools that have difficulties parsing spaces in paths, subjects, and template names. For example, the template name 'Corp!Signing!Cert' will be converted to 'Corp Signing Cert' in the function.
    $Delimiter = '!'
} # End of Begin block.
Process {
    ...
    If ($Path -and $Subject) {
        ...
        $Certificates = Get-ChildItem -Path "$($Path -replace $Delimiter, ' ')" | Where-Object { $PSItem.Subject -like "*$($Subject -replace $Delimiter, ' ')*" -or $PSItem.DnsNameList[0].Unicode -icontains $Subject }
    }
    ElseIf ($Path) {
